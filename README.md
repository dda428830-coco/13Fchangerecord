...
$env:SEC_USER_AGENT="13Fchangerecord your-name your-real-email@example.com"
py tg_push.py
```

`SEC_USER_AGENT` 必须包含真实可联系邮箱。SEC EDGAR 会拒绝默认、空白或明显无效的 User-Agent，常见表现是 `HTTP Error 403: Forbidden`。
...
这里的 `abcdef1234567890` 就是 `GIST_ID`。
...
- `DISCORD_WEBHOOK_URL`
- `GIST_ID`
- `SEC_USER_AGENT`，例如 `13Fchangerecord dda428830-coco your-real-email@example.com`
- 可选：`TG_BOT_TOKEN`
- 可选：`TG_CHAT_ID`
...
当前 GitHub Actions 配置为每月 1 日运行一次，并保留手动运行入口。

13F 是季度披露，通常在季度结束后最多 45 天内提交。每月检查一次可以减少 Discord 推送频率。
